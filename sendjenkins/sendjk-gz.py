...
def get_token(corp_id, secret):
    context = ssl._create_unverified_context() #增加https认证问题处理
    res = urllib2.urlopen('https://qyapi.weixin.qq.com/cgi-bin/gettoken?corpid=%s&corpsecret=%s' % (corp_id, secret),context = context)
...
def send_msg(content, to_user, to_party, to_tag, application_id, safe):
    ...
    try:
       ...
       # print data,type(data)
        ...
        resp = urllib2.urlopen(req,data,context = context)
        msg = 'send msg ok...'
    except Exception,ex:
        msg = u'异常:'+str(ex)
    finally:
